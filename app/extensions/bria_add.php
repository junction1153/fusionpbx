...
//includes
	...
	if (count($_POST) > 0) {
		if($_POST['extensions'] != "" && $_POST['extensions'] != 0 && $_POST['extension'] != "" && $_POST['password'] != "" && $_POST['emailaddress'] != "" && $_POST['sipprofile'] != "" &&  $_POST['group'] != "0"){
			...
			$uuid = uuid();
			$sql = "INSERT INTO bria_password_manage(uuid,password,count,time) VALUES ('".$uuid."','".$encrypted_password."','0','".$str_time."')";
			$database = new database;
			$parameters="";
...
//begin the page content
	...
	echo "<input class='formfld' type='text' name='extension' autocomplete='new-password' maxlength='255' value='' required='required'>";
	echo "@";
...
/*	echo "    <select class='formfld' name='group' id='group' onchange='profile_bria(this.value)'>\n";
	...
	echo"</td>";
	echo "<td  width='70%' class='vtable' align='left'>";
	echo "<input class='formfld' type='text' name='emailaddress' autocomplete='new-password' maxlength='255' value='' required='required'>";
	echo "    <br />\n";
	echo "</td>";
	echo "</tr>";
	echo "<tr>";
