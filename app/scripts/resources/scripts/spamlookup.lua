...
--get the variables
        ...
        if(caller_id_number ~= '')then
			...
			sql = sql .. "from spam_lookup ";
			local params = {};
			log.notice("spam_lookup_max: "..sql);
			local spam_lookup_max_row = dbh:first_row(sql, params)
			if not spam_lookup_max_row then 
				spam_lookup_max_row_inc_id = 1
			else
				log.notice("spam_lookup_max_row: "..spam_lookup_max_row.id);
				spam_lookup_max_row_inc_id = tonumber(spam_lookup_max_row.id)+1
			end
                                log.notice("phonebook INC ID HARSH: "..spam_lookup_max_row_inc_id);
				...
				id = spam_lookup_max_row_inc_id;
                        ...
                        if(id ~= nil)then
                                ...
                                log.notice("spam_lookup_SQL_IDs:"..id);
                                log.notice("spam_lookup_SQL:"..id);
                                if(isspam == '0')then
