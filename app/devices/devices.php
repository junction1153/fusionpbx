...
//JA TEMP	if (permission_exists('device_add') && (empty($_SESSION['limit']['devices']['numeric']) || ($total_devices < $_SESSION['limit']['devices']['numeric']))) {
...
//		unset($margin_left);
//	}
// temporary: only show + ADD to users in the superadmin group
if (
	if_group("superadmin") &&
	permission_exists('device_add') &&
	(empty($_SESSION['limit']['devices']['numeric']) || ($total_devices < $_SESSION['limit']['devices']['numeric']))
) {
	echo button::create(['type'=>'button','label'=>$text['button-add'],'icon'=>$_SESSION['theme']['button_icon_add'],'id'=>'btn_add','style'=>$margin_left,'link'=>'device_edit.php']);
	unset($margin_left);
}
