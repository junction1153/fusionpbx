...
if (count($_POST) > 0) {
    ...
    //HP:start
    $ztp_provision = $_POST["ztp_provision"];
    $ztp_reference_id = $_POST['ztp_reference_id'];
    $old_device_mac_address = $_POST['old_device_mac_address'];
    //HP:end
...
$custom_jcc_tls_ports = array("5182","5178","5192","5185","5172","5198","5175","5187","5125","5127","5061","5189");
$custom_jcc_udp_ports = array("5180","5176","5190","5184","5170","5196","5174","5186","5060","5124","5126","5188");
...
if ($row['sip_transport'] == "udp" && !in_array($row['sip_port'], $custom_jcc_udp_ports)) { $msg .= "ERROR: Please associate correct port with transport<br>\n"; }
    ...
    if ($action == 'add' && $device_address != "000000000000") {
        ...
        //Pooja: Start code for add ZTP-profile for polycom
        $template_string_array = explode("/", $device_template);
        if ($ztp_provision == true && isset($template_string_array[0]) && $template_string_array[0] == 'polycom') {
            $session_domain_name = $_SESSION['domain_name'];
            $session_domain_uuid = $_SESSION['domain_uuid'];
            $ztp_device_response = device::add_ZTP_profile("add", $session_domain_name, $session_domain_uuid, $device_address);
            if ($ztp_device_response['status'] == "Error") {
                message::add($ztp_device_response['message'], 'negative');
                header('Location: devices.php');
                exit;
            } else {
                $ztp_reference_id = $ztp_device_response['ztp_reference_id'];

            }
        }
        //Pooja: End code for add ZTP-profile for polycom
        ...
        if ($ztp_provision == true && isset($template_string_array[0]) && $template_string_array[0] == 'polycom') { 
            ...
            if ((isset($ztp_reference_id) && !empty($ztp_reference_id)) && ($device_address != $old_device_mac_address)) {
                $ztp_device_response = device::add_ZTP_profile("edit", $session_domain_name, $session_domain_uuid, $device_address, $ztp_reference_id, $old_device_mac_address);

            } elseif (!isset($ztp_reference_id) || (isset($ztp_reference_id) && empty($ztp_reference_id))) {
                $ztp_device_response = device::add_ZTP_profile("add", $session_domain_name, $session_domain_uuid, $device_address);
            ...
            if ($ztp_device_response['status'] == "Error") {
                ...
                $ztp_reference_id = isset($ztp_device_response['ztp_reference_id']) ? $ztp_device_response['ztp_reference_id'] : $ztp_reference_id;

            }

        } else {
            if ((isset($ztp_reference_id) && !empty($ztp_reference_id))) {

                $delete_ztp_device = device::delete_ZTP_profile($ztp_reference_id, $old_device_mac_address);
        ...
        //Pooja: End code for Edit ZTP-profile for polycom
    ...
    if (empty($_POST["persistformvar"]) || $_POST["persistformvar"] != "true") {
		...
		$array['devices'][0]['ztp_provision'] = $ztp_provision;
		$template_string_array = explode("/",$device_template);
		if((isset($ztp_reference_id) && !empty($ztp_reference_id) ) && $ztp_provision == true && isset($template_string_array[0]) && $template_string_array[0]=='polycom')
		{
			$array['devices'][0]['ztp_reference_id'] = $ztp_reference_id;
		} else {
			$array['devices'][0]['ztp_reference_id'] = "";
		}
...
if (!empty($_GET) && (empty($_POST["persistformvar"]) || $_POST["persistformvar"] != "true")) {
    ...
    if (is_array($row) && @sizeof($row) != 0) {
        $device_address = $row["device_address"];
	$old_device_mac_address = $row["device_address"];
        $device_provisioned_ip = $row["device_provisioned_ip"];
        $domain_uuid = $row["domain_uuid"];
        $device_label = $row["device_label"];
        $device_label = $row["device_label"];
		$ztp_provision = $row["ztp_provision"];
		$ztp_reference_id  = $row["ztp_reference_id"];
...
echo "</td>\n";
echo "</tr>\n";
if (permission_exists('device_ztp_mac_address')) {
	echo "<tr>\n";
	echo "<td class='vncell' valign='top' align='left' nowrap='nowrap'>\n";
	echo "ZTP Provision\n";
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "	<input class='formfld' type='hidden' name='ztp_reference_id' maxlength='255' value=\"".escape($ztp_reference_id)."\"/>\n";
	echo "	<input class='formfld' type='hidden' name='old_device_mac_address' maxlength='255' value=\"".escape($old_device_mac_address)."\"/>\n";
	
	echo "<input type='checkbox' name='ztp_provision' value='true' class='chk_delete checkbox_lines' ";
	if($ztp_provision == 'true')
	{
		echo " checked ";
	}
	echo  " \" >\n";
...
//JA
                                        ...
                                        $database = new database;
                                        $device_key_value_placeholder = $database->select($sql, $parameters, 'column');
                                        $row['device_key_value_placeholder'] = $device_key_value_placeholder;
                                        unset($sql, $parameters);
				echo "<td align='left'>\n";
				echo "	<input class='formfld' type='text' placeholder=\"".escape($row["device_key_value_placeholder"])."\" name='device_keys[".$x."][device_key_label]' style='width: 200px;' maxlength='255' value=\"".escape($row['device_key_label'])."\"/>\n";
				echo "</td>\n";
