...
//action add or update
	...
	if (count($_POST) > 0 ) {
		...
		if($domain_permissions_num_rows == 0){
			$sql = "insert into domain_permissions(group_uuid,domain_uuids) VALUES('".$group_uuid."','".$domain_checkbox_str_final."')";
		}else{
			$sql = "update domain_permissions set domain_uuids= '".$domain_checkbox_str_final."' where group_uuid ='".$group_uuid."' ";
		}
//echo $sql;exit;
                $database = new database;
